            ...
            @foreach($items as $label => $url)
                <li aria-current="{{ $loop->last ? 'page' : 'false' }}">
                    <div class="flex items-center">
                        ...
                        @if($loop->last)
                            <span class="ml-1 text-sm font-semibold text-gray-500 md:ml-2 truncate max-w-[150px] sm:max-w-xs" title="{{ $label }}">
                                {{ $label }}
                            </span>
                        @else
                            <a href="{{ $url }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 transition-colors duration-200">
                                {{ $label }}
                            </a>
                        @endif
                    </div>
